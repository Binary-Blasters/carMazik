...
import { Clock, CarFront, Loader2, Calendar, IndianRupee } from "lucide-react";
import { getSellerPendingCars } from "../../api/car";
...
const PendingCars = () => {
  ...
  useEffect(() => {
    const fetchPendingCars = async () => {
      try {
        const res = await getSellerPendingCars();
        setCars(res?.data || []);
      } catch (err) {
        setAlert({
          type: "error",
          ...
          message:
            ...
            "Unable to fetch pending cars right now. Please try again later.",
        });
      } finally {
        setLoading(false);
      }
  ...
  if (loading)
    return (
      <LoadingScreen message="Fetching your pending cars — just a moment 🚗💨" />
    );
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-blue-50 via-white to-orange-50 min-h-screen">
      <h2 className="text-3xl font-bold mb-8 bg-gradient-to-r from-blue-600 to-orange-500 bg-clip-text text-transparent flex items-center gap-3">
        <Clock className="text-orange-500 h-7 w-7" /> Pending Car Listings
      </h2>

      {cars.length === 0 ? (
        <div className="text-center text-gray-600 mt-20">
          <CarFront className="mx-auto h-16 w-16 text-gray-400 mb-3" />
          <p className="text-lg font-medium">No pending cars found!</p>
          <p className="text-sm text-gray-500">
            Uploaded cars awaiting admin approval will appear here.
          </p>
        </div>
      ) : (
          ...
          {cars.map((car) => (
            ...
            >
              <div className="relative">
                ...
                />
                <div className="absolute top-2 right-2 bg-yellow-100 text-yellow-800 text-xs font-semibold px-3 py-1 rounded-full">
                  Pending Review
                </div>
              </div>
              ...
              <div className="p-4">
                ...
                <div className="mt-4 flex justify-between items-center">
                  <span className="text-xs text-gray-500">
                    Uploaded: {new Date(car.createdAt).toLocaleDateString()}
                  </span>
